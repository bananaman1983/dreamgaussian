...
def optimize_stage_1(image_block: Image.Image, preprocess_chk: bool, elevation_slider: float):
    ...
    # stage 1
    subprocess.run(f'python main.py --config {os.path.join("configs", "image_sai.yaml")} input={os.path.join("tmp_data", "tmp_rgba.png")} save_path=tmp mesh_format=glb elevation={elevation_slider} force_cuda_rast=True', shell=True)
...
def optimize_stage_2(elevation_slider: float):
    # stage 2
    subprocess.run(f'python main2.py --config {os.path.join("configs", "image_sai.yaml")} input={os.path.join("tmp_data", "tmp_rgba.png")} save_path=tmp mesh_format=glb elevation={elevation_slider} force_cuda_rast=True', shell=True)
